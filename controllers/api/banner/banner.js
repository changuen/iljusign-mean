...
var storage = multer.diskStorage({
    destination: function(req, file, cb) {
        cb(null, './assets/images/items/banner');
    },
        ...
        return next(err);
		        ...
		        if (err) {
                  ...
                  res.json({ success: false, message: '선택된 파일이 없습니다.' });
											...
											var dbfilePath = '/images/items/banner/';
                      var item_path = dbfilePath + req.file.filename;

                      var insertSql = 'INSERT INTO banner set ?';
                      var insertValue = {
                      ...
                      connection.query(insertSql, insertValue, function (error, results, next) {
                          if(err){
                            ...
                            console.log(error);
                            console.log(results);
                              res.status(201).send({success:true, message:'배너 등록 성공!'});
                          }
                      });
			            }
			      	}
        ...
        var selectSql = 'select * from banner';
          ...
          if(err){
            ...
            if(!result){
              ...
              res.json({success:true, message:'정보를 불러왔습니다.', bannerData: result});
            }
          }
        });
        }
